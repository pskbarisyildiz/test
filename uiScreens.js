...
function renderSetupScreen(app) {
            ...
            <div id="singleMatchMode" style="display: block;">
                <div style="max-width: 900px; margin: 0 auto; background: rgba(0,0,0,0.5); backdrop-filter: blur(20px); border-radius: 24px; padding: 40px; border: 1px solid rgba(255,255,255,0.1);">
                    <div style="margin-bottom: 30px; padding: 20px; background: rgba(16,185,129,0.1); border-radius: 16px; border: 1px solid rgba(16,185,129,0.3);">
                        ...
                        <div style="font-size: 14px; margin-top: 8px; opacity: 0.9;">${gameState.teams.length} teams • ${gameState.players.length} professional players</div>
                    </div>
                    ...
                    <div class="grid grid-2">
                        <div>
                            ...
                            <select id="homeSelect" class="select">
                                ${gameState.teams.map(t => `<option value="${t}" ${t === gameState.homeTeam ? 'selected' : ''}>${t}</option>`).join('')}
                            </select>
                            
                            <label class="label" style="margin-top: 20px;">Tactic</label>
                            <select id="homeTacticSelect" class="select">
                                ${Object.keys(TACTICS).map(key => 
                                    `<option value="${key}" ${key === gameState.homeTactic ? 'selected' : ''}>${TACTICS[key].name}</option>`
                                ).join('')}
                            </select>
                        </div>
                        <div>
                            ...
                            <select id="awaySelect" class="select">
                                ${gameState.teams.map(t => `<option value="${t}" ${t === gameState.awayTeam ? 'selected' : ''}>${t}</option>`).join('')}
                            </select>
                            
                            <label class="label" style="margin-top: 20px;">Tactic</label>
                            <select id="awayTacticSelect" class="select">
                                ${Object.keys(TACTICS).map(key => 
                                    `<option value="${key}" ${key === gameState.awayTactic ? 'selected' : ''}>${TACTICS[key].name}</option>`
                                ).join('')}
                            </select>
                        </div>
                    </div>
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr auto; gap: 15px; align-items: end;">
                            <div>
                                ...
                                <select id="batchHomeSelect" class="select">
                                    ${gameState.teams.map(t => `<option value="${t}">${t}</option>`).join('')}
                                </select>
                            </div>
                            <div>
    ...
    setTimeout(attachSetupEventListeners, 0);
}
...
function addMatchToBatch() {
    ...
    CustomFixtureSimulator.addMatch(homeTeam, awayTeam);
}