...
const ThrowInBehaviors = {
    getThrowInPosition(player, throwPos, ownGoalX, opponentGoalX, gameState, teammates, opponents) {
        ...
        const context = new TacticalContext(gameState, SET_PIECE_TYPES.THROW_IN);
        ...
        // Get player's tactical mentality
        const mentality = getPlayerTacticalMentality(player);
        const shouldCommit = shouldPlayerCommitForward(player, gameState, true);

        if (!gameState._throwInPosManager) gameState._throwInPosManager = new PositionManager();
        ...
        if (!gameState[setupKey] || gameState[setupKey] !== gameState.setPiece) {
            ...
            const infieldDepth = isInAttackingThird ? 70 : (isInMiddleThird ? 50 : 40);
            ...
            const throwerCandidates = validTeammates
                ...
                .sort((a, b) => window.getDistance(a, throwPos) - window.getDistance(b, throwPos));

            const thrower = throwerCandidates[0] || (validTeammates.length > 0
                ? validTeammates.sort((a, b) => window.getDistance(a, throwPos) - window.getDistance(b, throwPos))[0]
                : null);
    ...
    getDefendingThrowInPosition(player, throwPos, ownGoalX, gameState, teammates) {
        ...
        if (!gameState._throwInDefPosManager) gameState._throwInDefPosManager = new PositionManager();
