...
function assessGoalkeeperThreats(goalkeeper, ball, opponents) {
...
function determineGoalkeeperStance(goalkeeper, mainThreat, threats, ball) {
    if (!mainThreat) {
...
function calculateOptimalGoalkeeperPosition(goalkeeper, mainThreat, stance, ball) {
    ...
    if (gameState.status === 'CORNER_KICK' && gameState.setPiece) {
        ...
        const cornerX = gameState.setPiece.position.x;
        ...
        const attackersInBox = [...gameState.homePlayers, ...gameState.awayPlayers]
        ...
        // Calculate optimal Y position based on corner location
        ...
        const postAdjustment = isNearPost ? 0 : 10; // Slight adjustment for far post
...
function shouldGoalkeeperSweep(goalkeeper, ball, opponents) {
    // Is ball trajectory heading toward goal?
    if (!gameState.ballTrajectory || gameState.ballTrajectory.isShot) {
        return false;
    }
    
    const goalX = getAttackingGoalX(!goalkeeper.isHome, goalkeeper.currentHalf);
 ...
 */
function handleCrossSituation(goalkeeper, ball, opponents) {
...
function updateGoalkeeperAI_Advanced(goalkeeper, ball, opponents) {
    ...
    if (shouldGoalkeeperSweep(goalkeeper, ball, opponents)) {
        ...
        console.log(`ðŸƒ ${goalkeeper.name} sweeping!`);
        return;
    }
...
function triggerGoalkeeperSave(goalkeeper, shotX, shotY, saveProbability = 0.5) {
    ...
    // --- BASIC DIVE SETUP (kept same as your code) ---
    ...
    const distance = Math.sqrt(diveX ** 2 + diveY ** 2);
    const maxReach = 180;
    const reachFactor = Math.min(1, distance / maxReach);
    ...
    // --- AUTO RESET (same as original, just using dynamic time) ---
    setTimeout(() => {
        goalkeeper.isDiving = false;
    }, diveDuration);

    // --- PARTICLES (compatible with your system) ---
    const numParticles = Math.floor(8 + Math.random() * 4);
    const particleColor = saveProbability > 0.6 ? '#60a5fa' : '#1e40af';

    if (typeof Particle !== 'undefined' && gameState?.particles) {
...
function resolveShot_WithAdvancedGK({ holder, xG, goalkeeper, goalX, shotTargetY }) {
    ...
    const saveProbability = calculateSaveProbability_Advanced(xG, goalkeeper, shotTargetY);
    ...
    if (!keeperSaves) {
        ...
        // --- THIS IS THE FIX ---
        // It was "team: holder.isHome ? 'home' : 'away'"
        // It must be "isHome: holder.isHome"
        ...
        // --- SAVED ---
        triggerGoalkeeperSave(goalkeeper, goalX, shotTargetY);
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = {
        GOALKEEPER_CONFIG,
        assessGoalkeeperThreats,
        determineGoalkeeperStance,
        calculateOptimalGoalkeeperPosition,
        shouldGoalkeeperSweep,
        handleCrossSituation,
        updateGoalkeeperAI_Advanced,
        calculateSaveProbability_Advanced,
        triggerGoalkeeperSave,
        drawGoalkeeperStanceIndicator,
        resolveShot_WithAdvancedGK,
    };
}
