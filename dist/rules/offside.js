...
export function recordOffsidePositions(passingPlayer, allPlayers) {
    ...
    const teammates = allPlayers.filter(p => p.isHome === passingPlayer.isHome &&
...
export function checkOffsidePenalty(player) {
...
export function awardOffsideFreeKick(offsidePlayer) {
...
export function shouldAvoidOffside(player, ball, opponents) {
    const opponentGoalX = getAttackingGoalX(player.isHome, gameState.currentHalf);
