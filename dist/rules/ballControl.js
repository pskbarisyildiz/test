...
export function resolveBallControl(allPlayers) {
...
export function canControlBall(player, ball) {
    ...
    if (isSetPiece && gameState.setPiece && !gameState.setPiece.executed) {
        const isDesignatedTaker = player.setPieceRole && (player.setPieceRole.includes('KICKER') ||
...
export function action_attemptTackle(player, allPlayers) {
...
export function handleBallInterception(progress) {
    ...
    if (isHeaderOpportunity) {
        const contenders = allPlayers.filter(player => !(player.stunnedUntil && Date.now() < player.stunnedUntil)).map(player => ({
            player,
...
//# sourceMappingURL=ballControl.js.map