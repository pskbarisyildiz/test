...
// UPLOAD SCREEN
// ============================================================================
export function renderUploadScreen(app) {
    app.innerHTML = `
        <div style="
            min-height: 100vh;
            display: flex;
...
export function renderSetupScreen(app) {
    ...
    const tacticsOptions = Object.keys(TACTICS).map(key => `<option value="${key}" ${key === gameState.homeTactic ? 'selected' : ''}>${TACTICS[key]?.name || key}</option>`).join('');
    const awayTacticsOptions = Object.keys(TACTICS).map(key => `<option value="${key}" ${key === gameState.awayTactic ? 'selected' : ''}>${TACTICS[key]?.name || key}</option>`).join('');
    const teamsOptions = gameState.teams ? gameState.teams.map((t) => `<option value="${t}" ${t === gameState.homeTeam ? 'selected' : ''}>${t}</option>`).join('') : '';
    const awayTeamsOptions = gameState.teams ? gameState.teams.map((t) => `<option value="${t}" ${t === gameState.awayTeam ? 'selected' : ''}>${t}</option>`).join('') : '';
    const batchHomeTeamsOptions = gameState.teams ? gameState.teams.map((t) => `<option value="${t}">${t}</option>`).join('') : '';
    const batchAwayTeamsOptions = gameState.teams ? gameState.teams.map((t) => `<option value="${t}">${t}</option>`).join('') : '';
    app.innerHTML = `
        <div class="container">
...
// ============================================================================
export function switchSimulationMode(mode) {
...
// BATCH MATCH HELPER
// ============================================================================
export function addMatchToBatch() {
...
//# sourceMappingURL=uiScreens.js.map