...
export const GoalkeeperBehaviors = {
    ...
    sweeperKeeping(_gk, ball, teammates, opponents, ownGoalX) {
        ...
        if (defenders.length === 0)
            return null; // No defenders = no sweeping needed
...
export function selectPlayerBehavior(player, gameState, phase, tacticalSystem) {
    ...
    if (phase === PHASES.TRANSITION_TO_ATTACK || justWonPossession) {
        behaviors.push(TransitionBehaviors.counterAttackRun(player, ball, opponentGoalX, justWonPossession));
    }
    // PHASE 2: TRANSITION TO DEFENSE
    if (phase === PHASES.TRANSITION_TO_DEFENSE || justLostPossession) {
        behaviors.push(TransitionBehaviors.counterPress(player, ball, justLostPossession, opponentGoalX), TransitionBehaviors.recoveryRun(player, ball, ownGoalX, justLostPossession));
    }
    // PHASE 3: DEFENSIVE PHASE
    if (phase === PHASES.DEFENSIVE || (holder && holder.isHome !== player.isHome)) {
        // Goalkeeper behaviors
        if (player.role === 'GK') {
            behaviors.push(GoalkeeperBehaviors.sweeperKeeping(player, ball, teammates, opponents, ownGoalX), GoalkeeperBehaviors.angleNarrowing(player, ball, holder, ownGoalX));
        }
        // Defensive unit behaviors
        behaviors.push(DefensiveBehaviors.defensiveLineShift(player, ball, teammates, ownGoalX), DefensiveBehaviors.fullBackCovering(player, ball, opponents, ownGoalX), DefensiveBehaviors.centerBackMarking(player, opponents, teammates, ownGoalX));
        // Midfield defensive behaviors
        behaviors.push(MidfieldBehaviors.cdmScreening(player, ball, opponents, teammates, ownGoalX), MidfieldBehaviors.cmPressTrigger(player, ball, opponents, holder));
    ...
    if (phase === PHASES.ATTACKING || (holder && holder.isHome === player.isHome)) {
        // Goalkeeper behaviors
        if (player.role === 'GK') {
            behaviors.push(GoalkeeperBehaviors.buildUpSupport(player, holder, teammates, ownGoalX));
        }
        ...
        behaviors.push(MidfieldBehaviors.boxToBoxLateRun(player, ball, holder, teammates, opponentGoalX), MidfieldBehaviors.registaTempoDictation(player, holder, teammates, ownGoalX), MidfieldBehaviors.camBetweenLines(player, opponents, opponentGoalX));
        // Forward attacking behaviors
        behaviors.push(ForwardBehaviors.wingerWidthProviding(player, holder, teammates), ForwardBehaviors.invertedWingerCutInside(player, ball, opponentGoalX), ForwardBehaviors.strikerRunsInBehind(player, holder, opponents, opponentGoalX), ForwardBehaviors.false9DropDeep(player, holder, teammates, opponentGoalX), ForwardBehaviors.targetStrikerHoldUp(player, ball, opponents));
    }
    ...
    if (tacticalSystem === 'possession') {
        ...
        behaviors.push(TacticalSystemModifiers.highPressSystem(player, ball, teammates, opponentGoalX));
    }
    else if (tacticalSystem === 'low_block' || tacticalSystem === 'counter_attack') {
        behaviors.push(TacticalSystemModifiers.lowBlockSystem(player, ownGoalX, teammates));
    }
    // Find first valid behavior
    const validBehavior = behaviors.find(b => b !== null);
    if (validBehavior)
        player.currentBehavior = validBehavior.description; // Debug info
