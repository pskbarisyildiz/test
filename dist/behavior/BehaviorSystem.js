...
const DefensiveBehaviors = {
    defensiveLineShift(player, ball, teammates, ownGoalX) {
        if (!['CB', 'RB', 'LB'].includes(player.role))
            return null;
        const defensiveLine = teammates.filter(t => ['CB', 'RB', 'LB'].includes(t.role));
    ...
    fullBackCovering(fb, opponents) {
        ...
        const wideThreats = opponents.filter(opp => (isRightBack && opp.y < 200) || (!isRightBack && opp.y > 400));
        if (wideThreats.length === 0)
            return null;
        const nearestThreat = wideThreats.sort((a, b) => getDistance(a, fb) - getDistance(b, fb))[0];
        if (!nearestThreat)
            return null;
    ...
    centerBackMarking(cb, opponents, ownGoalX) {
        ...
        if (strikers.length === 0)
            return null;
        const nearestStriker = strikers.sort((a, b) => getDistance(a, cb) - getDistance(b, cb))[0];
        if (!nearestStriker)
            return null;
    ...
    invertedFullBack(fb, holder, teammates, ownGoalX) {
        ...
        if (!ballInOwnHalf)
            return null;
        const midfielders = teammates.filter(t => ['CM', 'CDM', 'CAM'].includes(t.role));
        const midFieldCongestion = midfielders.reduce((sum, m) => sum + (Math.abs(m.y - 300) < 100 ? 1 : 0), 0);
...
const MidfieldBehaviors = {
    ...
    boxToBoxLateRun(cm, ball, holder, teammates, opponentGoalX) {
        ...
        const playersInBox = teammates.filter(t => Math.abs(t.x - opponentGoalX) < 150).length;
    ...
    camBetweenLines(cam, opponents, opponentGoalX) {
        if (!['CAM'].includes(cam.role))
            return null;
        const oppDefenders = opponents.filter(o => ['CB', 'RB', 'LB', 'CDM'].includes(o.role));
...
const ForwardBehaviors = {
    ...
    invertedWingerCutInside(winger, opponentGoalX, opponents, _ball) {
        ...
        if (distToGoal < 280 && isWide) {
            ...
            // OFFSIDE CHECK: Find the last defender (excluding GK)
            ...
            if (oppDefenders.length > 0) {
                ...
                const sortedDefenders = oppDefenders.sort((a, b) => Math.abs(a.x - opponentGoalX) - Math.abs(b.x - opponentGoalX));
    ...
    strikerRunsInBehind(striker, holder, opponents, opponentGoalX, _ball) {
        ...
        if (!holder || holder.isHome !== striker.isHome)
            return null;
        const oppDefenders = opponents.filter(o => ['CB', 'RB', 'LB'].includes(o.role));
        if (oppDefenders.length === 0)
            return null;
        const lastDefender = oppDefenders.sort((a, b) => Math.abs(b.x - opponentGoalX) - Math.abs(a.x - opponentGoalX))[0];
        if (!lastDefender)
            return null;
        ...
        if (distToLastDefender < 20 || getDistance(holder, striker) > 250)
            return null;
        return {
            target: {
                x: runTargetX,
    ...
    false9DropDeep(striker, holder, teammates, opponentGoalX) {
        ...
        if (!holder || holder.isHome !== striker.isHome)
            return null;
        const forwardRunners = teammates.filter(t => ['RW', 'LW', 'CAM'].includes(t.role) &&
            Math.abs(t.vx) > 50);
    ...
    targetStrikerHoldUp(striker, opponents, ball) {
        if (!['ST', 'CF'].includes(striker.role))
            return null;
        const nearbyOpponents = opponents.filter(o => getDistance(o, striker) < 60);
    ...
    strikerPressingTrigger(striker, opponents, ball, ownGoalX) {
        if (!['ST', 'CF'].includes(striker.role))
            return null;
        const oppCBs = opponents.filter(o => ['CB', 'GK'].includes(o.role));
        if (oppCBs.length === 0)
            return null;
        const nearestCB = oppCBs.sort((a, b) => getDistance(a, ball) - getDistance(b, ball))[0];
        if (!nearestCB)
            return null;
...
const TransitionBehaviors = {
    counterAttackRun(player, opponentGoalX, justWonPossession, opponents, _ball) {
        ...
        if (['ST', 'RW', 'LW'].includes(player.role) && spaceAhead > 150) {
            ...
            // OFFSIDE CHECK: Find the last defender
            const oppDefenders = opponents.filter(o => o.role !== 'GK');
...
const TacticalSystemModifiers = {
    possessionSystem(player, holder, teammates) {
        if (!holder || holder.isHome !== player.isHome)
            return null;
        const nearbyTeammates = teammates.filter(t => getDistance(t, holder) < 150 && t.id !== player.id && t.id !== holder.id);
...
export function selectPlayerBehavior(player, gameState, phase, tacticalSystem) {
    ...
    if (phase === PHASES.TRANSITION_TO_ATTACK || justWonPossession) {
        behaviors.push(TransitionBehaviors.counterAttackRun(player, opponentGoalX, justWonPossession, opponents, ball));
    }
    if (phase === PHASES.TRANSITION_TO_DEFENSE || justLostPossession) {
        behaviors.push(TransitionBehaviors.counterPress(player, ball, justLostPossession), TransitionBehaviors.recoveryRun(player, ball, ownGoalX, justLostPossession));
    }
    if (phase === PHASES.DEFENSIVE || (holder && holder.isHome !== player.isHome)) {
        if (player.role === 'GK') {
            behaviors.push(GoalkeeperBehaviors.sweeperKeeping(ball, teammates, opponents, ownGoalX), GoalkeeperBehaviors.angleNarrowing(ball, ownGoalX));
        }
        behaviors.push(DefensiveBehaviors.defensiveLineShift(player, ball, teammates, ownGoalX), DefensiveBehaviors.fullBackCovering(player, opponents), DefensiveBehaviors.centerBackMarking(player, opponents, ownGoalX));
        behaviors.push(MidfieldBehaviors.cdmScreening(ball, teammates), MidfieldBehaviors.cmPressTrigger(player, holder));
        behaviors.push(ForwardBehaviors.strikerPressingTrigger(player, opponents, ball, ownGoalX));
    }
    if (phase === PHASES.ATTACKING || (holder && holder.isHome === player.isHome)) {
        if (player.role === 'GK') {
            behaviors.push(GoalkeeperBehaviors.buildUpSupport(holder, ownGoalX));
        }
        behaviors.push(DefensiveBehaviors.invertedFullBack(player, holder, teammates, ownGoalX));
        behaviors.push(MidfieldBehaviors.boxToBoxLateRun(player, ball, holder, teammates, opponentGoalX), MidfieldBehaviors.registaTempoDictation(player, holder, ownGoalX), MidfieldBehaviors.camBetweenLines(player, opponents, opponentGoalX));
        behaviors.push(ForwardBehaviors.wingerWidthProviding(player, holder), ForwardBehaviors.invertedWingerCutInside(player, opponentGoalX, opponents, ball), ForwardBehaviors.strikerRunsInBehind(player, holder, opponents, opponentGoalX, ball), ForwardBehaviors.false9DropDeep(player, holder, teammates, opponentGoalX), ForwardBehaviors.targetStrikerHoldUp(player, opponents, ball));
    }
    if (tacticalSystem === 'possession') {
        behaviors.push(TacticalSystemModifiers.possessionSystem(player, holder, teammates));
    }
    else if (tacticalSystem === 'high_press') {
        behaviors.push(TacticalSystemModifiers.highPressSystem(player, opponentGoalX));
    }
    else if (tacticalSystem === 'low_block' || tacticalSystem === 'counter_attack') {
        behaviors.push(TacticalSystemModifiers.lowBlockSystem(player, ownGoalX));
    }
...
export function detectGamePhase(gameState) {
...
export function getTacticalSystemType(tacticName) {
    const systemMap = {
