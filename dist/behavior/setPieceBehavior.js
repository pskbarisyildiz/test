 ...
 */
import { sanitizePosition, getValidPlayers, getSortedLists, isPlayerAttacking, determineSetPieceTeam, getFormationAwarePosition, checkAndAdjustOffsidePosition, checkAndAdjustOffsidePositionWithAudit, PenaltyKickBehaviors, KickoffBehaviors, ThrowInBehaviors, ProfessionalCornerBehaviors, ProfessionalGoalKickBehaviors, ProfessionalFreeKickBehaviors } from '../setpieces';
...
function getSafeFallbackPosition(player, reason, gameState) {
    ...
    return sanitizePosition({ x: activePos?.x ?? PITCH_WIDTH / 2, y: activePos?.y ?? PITCH_HEIGHT / 2, movement: `fallback_${reason}` }, { player, behavior: 'Fallback' });
}
        ...
        switch (setPieceType) {
            ...
            case SET_PIECE_TYPES.CORNER_KICK:
                return isAttacking ?
                    ...
                    ProfessionalCornerBehaviors.getDefendingCornerPosition(player, setPiecePos, ownGoalX, opponents, null, gameState.setPiece?.defensiveSystem || 'man_marking', gameState, teammates);
...
function shouldLockSetPiecePosition(player, gameState) {
    if (!gameState || !player || !gameState.setPiece)
        return false;
...
//# sourceMappingURL=setPieceBehavior.js.map