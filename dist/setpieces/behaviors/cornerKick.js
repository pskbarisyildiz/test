...
export const ProfessionalCornerBehaviors = {
    getAttackingCornerPosition(player, cornerPos, opponentGoalX, teammates, sortedLists, _routine, gameState) {
        ...
        if (!gameState._cornerPosManager)
            gameState._cornerPosManager = new PositionManager();
        ...
        if (!gameState[setupKey] || gameState[setupKey] !== gameState.setPiece) {
            ...
            const decoyRunner = validTeammates.find(p => !assigned.has(String(p.id)) &&
    ...
    getDefendingCornerPosition(player, cornerPos, ownGoalX, opponents, sortedLists, system, gameState, teammates) {
        ...
        if (!gameState._cornerDefPosManager)
            gameState._cornerDefPosManager = new PositionManager();
...
//# sourceMappingURL=cornerKick.js.map