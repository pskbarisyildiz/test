 ...
 */
import { sanitizePosition, TacticalContext, PositionManager, getValidPlayers, getSortedLists, checkAndAdjustOffsidePosition } from '../utils';
...
export const ProfessionalFreeKickBehaviors = {
    getAttackingFreeKickPosition(player, fkPos, opponentGoalX, distToGoal, sortedLists, gameState, teammates) {
        ...
        if (!sortedLists)
            sortedLists = getSortedLists(teammates, getValidPlayers(gameState ? (player.isHome ? gameState.awayPlayers : gameState.homePlayers) : []));
        ...
        if (!gameState._fkAttPosManager)
            gameState._fkAttPosManager = new PositionManager();
        const posManager = gameState._fkAttPosManager;
        ...
        if (!gameState[setupKey] || gameState[setupKey] !== gameState.setPiece) {
            ...
            gameState._fkJobAssignments = playerJobs;
        }
        const playerIdStr = String(player.id);
        const myPositionData = gameState._fkJobAssignments?.get(playerIdStr);
        if (myPositionData) {
            ...
            const adjustedPosition = checkAndAdjustOffsidePosition(myPositionData, player, opponentGoalX, opponents, gameState);
    ...
    getDefendingFreeKickPosition(player, fkPos, ownGoalX, distToGoal, sortedLists, opponents, gameState, teammates) {
        if (!gameState || !player || !fkPos) {
        ...
        if (!gameState._fkDefPosManager)
            gameState._fkDefPosManager = new PositionManager();
        const posManager = gameState._fkDefPosManager;
        ...
        if (!gameState[setupKey] || gameState[setupKey] !== gameState.setPiece) {
            ...
            gameState._fkOpponentMap = new Map(validOpponents.map(p => [String(p.id), p]));
            gameState._currentWallSize = wallSize;
            const assigned = new Set();
            // Assign wall players (tallest/strongest)
            if (needsWall) {
                const wallPlayers = [...sortedLists.teammates.bestDefenders]
                    .slice(0, wallSize)
                    .sort((a, b) => getSafeStat(b.stats, 'heading', 70) - getSafeStat(a.stats, 'heading', 70));
            ...
            const zonalAssignments = isDangerous
                ? [{ zone: 'centralZone', role: 'CENTRAL_ZONE', priority: 7 }] // Only 1 for dangerous
                : [
            ...
            gameState._fkDefJobAssignments = playerJobs;
        }
        const playerIdStr = String(player.id);
        const myPositionData = gameState._fkDefJobAssignments?.get(playerIdStr);
        if (myPositionData) {
            ...
            if (finalPos.role?.startsWith('JOB_WALL_')) {
                ...
                finalPos = this.calculateMarkingPosition(finalPos, direction, gameState._fkOpponentMap, player, gameState, ownGoalX);
            }
            // Encroachment rule
    ...
    calculateWallPosition(positionData, wallSize, fkPos, ownGoalX, direction, player, gameState) {
        try {
    ...
    calculateMarkingPosition(positionData, direction, opponentMap, player, gameState, ownGoalX) {
        try {
...
//# sourceMappingURL=freeKick.js.map