...
// ============================================================================
export function executeCornerKick_Enhanced(gameState) {
    ...
    const routine = setPiece.routine || 'standard'; // 'cornerRoutine' yerine
...
// ============================================================================
export function executeFreeKick_Enhanced(gameState) {
    if (!gameState || !gameState.setPiece || !gameState.setPiece.position)
        return false;
    const setPiece = gameState.setPiece;
    // Validate ball position
    ...
    const hasWall = wallPlayers.length >= (gameState._currentWallSize || 3); // Kurulumdaki baraj boyutunu kontrol et
    if (isShootingRange && isGoodAngle) {
        ...
        if (Math.random() < shootingChance) {
            ...
            if (typeof passBall === 'function') {
                passBall(taker, setPiece.position.x, setPiece.position.y, opponentGoalX, targetY, accuracy, speed, true); // `true` = şut
    ...
    if (target && typeof passBall === 'function') {
        ...
        passBall(taker, setPiece.position.x, setPiece.position.y, target.x, target.y, passAccuracy, passSpeed, false); // `false` = pas
...
// ============================================================================
export function executeThrowIn_Enhanced(gameState) {
    if (!gameState || !gameState.setPiece || !gameState.setPiece.position)
        return false;
    const setPiece = gameState.setPiece;
    // Validate ball position
    ...
    let thrower = takingTeamPlayers.find(p => gameState._throwInAssignments?.get(String(p.id))?.role === 'THROWER');
    if (!thrower) {
        // Prioritize fullbacks and wingbacks (best for throw-ins)
        const wideDefenders = takingTeamPlayers.filter(p => ['RB', 'LB', 'RWB', 'LWB'].some(role => p.role.includes(role)));
        if (wideDefenders.length > 0) {
            // Pick closest wide defender to throw position
            thrower = wideDefenders.sort((a, b) => getDistance(a, setPiece.position) - getDistance(b, setPiece.position))[0];
        }
        else {
    ...
    const receivers = takingTeamPlayers
        ...
        if (inRange) {
            ...
            if (gameState._throwInAssignments?.get(String(p.id))?.role === 'SHORT_OPTION') {
                score += 50;
            }
        }
    ...
    if (target && typeof passBall === 'function') {
        ...
        passBall(thrower, setPiece.position.x, setPiece.position.y, target.x, target.y, accuracy, Math.min(throwPower, 600), false); // Max hız 600
        console.log(`⚽ Throw-in by ${thrower.name} to ${target.name}`);
        return true;
    }
    else {
        console.warn("Taç: Uygun alıcı bulunamadı.");
    }
    return false;
}
// ============================================================================
...
// ============================================================================
export function executeGoalKick_Enhanced(gameState) {
    // --- 1. SETUP (Gerekli) ---
    if (!gameState || !gameState.setPiece || !gameState.setPiece.position) {
    ...
    if (playShort && Math.random() < 0.75) {
        ...
        if (shortTargets.length > 0 && shortTargets[0] && shortTargets[0].space > 30) {
            ...
            passBall(goalkeeper, setPiece.position.x, setPiece.position.y, target.x, target.y, accuracy, Math.min(speed, 550), false);
    ...
    passBall(goalkeeper, setPiece.position.x, setPiece.position.y, targetX, targetY, accuracy, speed, false);
...
// ============================================================================
export function executeKickOff_Enhanced(gameState) {
    ...
    gameState.kickOffCompletedTime = Date.now();
    gameState.postKickOffCalmPeriod = true;
...
// SET PIECE ROUTER
// ============================================================================
export function executeSetPiece_Router(gameState) {
    if (!gameState || !gameState.setPiece) {
        console.warn("⚠️ Set Piece Router: Invalid game state or setPiece");
        return;
    }
    // ✅ FIX #7: Prevent duplicate execution
    if (gameState.setPiece.executed || gameState.setPieceExecuting) {
    ...
    try {
        // Set execution flag
        gameState.setPieceExecuting = true;
