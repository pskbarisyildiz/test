 ...
 */
export function assessGoalkeeperThreats(goalkeeper, _ball, opponents) {
 ...
 */
export function determineGoalkeeperStance(_goalkeeper, mainThreat, threats, _ball) {
    // Using imported gameState from globalExports
    if (!mainThreat) {
 ...
 */
export function calculateOptimalGoalkeeperPosition(goalkeeper, mainThreat, stance, ball) {
    // Using imported gameState from globalExports
 ...
 */
export function shouldGoalkeeperSweep(goalkeeper, _ball, opponents) {
    // Using imported gameState from globalExports
    ...
    const attackersChasingBall = opponents.filter(opp => {
        const distToBallTarget = Math.sqrt(Math.pow(opp.x - traj.endX, 2) +
    ...
    const attackerDistToBall = Math.min(...attackersChasingBall.map(att => getDistance(att, { x: traj.endX, y: traj.endY })));
 ...
 */
export function handleCrossSituation(goalkeeper, _ball, _opponents) {
    // Using imported gameState from globalExports
 ...
 */
export function updateGoalkeeperAI_Advanced(goalkeeper, ball, opponents) {
    // Using imported gameState from globalExports
    ...
    const optimalPosition = calculateOptimalGoalkeeperPosition(goalkeeper, mainThreat, stance, ball);
 ...
 * - xG value
 */
export function calculateSaveProbability_Advanced(xG, goalkeeper, shotTargetY, shooter) {
    if (!goalkeeper)
        return 0.3;
    ...
    if (shooter) {
        ...
        const finishingPower = (finisherSkill * 0.5 +
            dribbleQuality * 0.2 +
            shotAccuracy * 0.2 +
            xGOTFactor * 0.1);
        // Balance vs GK
    ...
    console.log(`SAVE%: ${(saveProbability * 100).toFixed(0)} | xG:${(xG * 100).toFixed(0)} | GK:${(baseAbility * 100).toFixed(0)} | ShooterAdj:${(attackerEffect * 100).toFixed(0)}%`);
 ...
 */
export function triggerGoalkeeperSave(goalkeeper, shotX, shotY, saveProbability = 0.5) {
    if (!goalkeeper)
        return;
    // Using imported gameState from globalExports
    // --- BASIC DIVE SETUP ---
    ...
    if (gameState?.particles) {
        for (let i = 0; i < numParticles; i++) {
            ...
            const particle = new Particle(goalkeeper.x, goalkeeper.y, particleColor, velocityX, velocityY);
 ...
 */
export function drawGoalkeeperStanceIndicator(ctx, goalkeeper) {
 ...
 */
export function resolveShot_WithAdvancedGK(params) {
    // Using imported gameState from globalExports
    // Using imported GAME_CONFIG from config
