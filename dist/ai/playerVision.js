...
export function getPlayerFacingDirection(player) {
    if (player.role === 'GK') {
        ...
        if (gameState.ballPosition) {
            ...
            if (distToBall < 350) {
                return Math.atan2(gameState.ballPosition.y - player.y, gameState.ballPosition.x - player.x);
            }
        }
        const opponentGoalX = getAttackingGoalX(player.isHome, gameState.currentHalf);
        return Math.atan2(300 - player.y, opponentGoalX - player.x);
    }
    if (player.vx !== 0 || player.vy !== 0) {
        const speed = Math.sqrt(player.vx * player.vx + player.vy * player.vy);
        if (speed > 10) {
...
export function canPlayerSee(player, target, detailLevel = 'full') {
    // Distance check
...
export function updatePlayerScanning(player) {
    const now = Date.now();
    if (!player.scanState) {
        player.scanState = {
            lastScanTime: now,
...
export function getVisibleTeammates(player, allPlayers) {
...
export function findBestPassOption_WithVision(passer, teammates, opponents) {
...
export function getPerceivedThreats(player, opponents) {
...
export function drawVisionCones(ctx) {
    ...
    allPlayers.forEach(player => {
        ...
        ctx.arc(player.x, player.y, VISION_CONFIG.MAX_VISION_RANGE, facingAngle - visionAngle / 2, facingAngle + visionAngle / 2);
        ctx.closePath();
        ctx.fill();
        ...
        ctx.beginPath();
        ctx.moveTo(player.x, player.y);
        ctx.lineTo(player.x + Math.cos(facingAngle) * 40, player.y + Math.sin(facingAngle) * 40);
        ctx.stroke();
        ctx.restore();
    });
}
//# sourceMappingURL=playerVision.js.map