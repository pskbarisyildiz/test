 ...
 */
export function getPlayerActivePosition(player, currentHalf) {
 ...
 */
export function getZoneForPlayer(player, _activePosition, teamState) {
    ...
    let zone = { x1: 0, y1: 0, x2: FIELD_WIDTH, y2: FIELD_HEIGHT }; // Default
 ...
 * - Marking coverage
 */
export function assessDefensiveThreats(defendingPlayer, opponents, ownGoalX) {
    // Using imported gameState from globalExports
 ...
 */
export function findMostDangerousAttacker(player, threats, playerZone) {
    ...
    // 4. Zonal emergency
    const threatsInZone = threats.filter(t => t.player.x > playerZone.x1 && t.player.x < playerZone.x2 &&
 ...
 */
export function calculateOptimalMarkingPosition(marker, target, ownGoalX, tightness = 'goal_side') {
 ...
 */
export function updateTacticalPosition(player, ball, _teammates, opponents) {
    // Using imported gameState from globalExports
    ...
    const now = Date.now();
    const distToTarget = Math.sqrt(Math.pow(player.targetX - player.x, 2) +
 ...
 */
export function applyMarkingAndPressing(player, _ball, opponents, activePosition, ownGoalX, tactic, teamState) {
    // Using imported gameState from globalExports
    const ballCarrier = gameState.ballHolder;
    if (!ballCarrier)
 ...
 */
export function applyDefensivePositioning(player, ball, _tactic, activePosition, ownGoalX, teamState) {
    // Using imported gameState from globalExports
 ...
 */
export function applyAttackingMovement(player, holder, teammates, activePosition, opponentGoalX, _teamState) {
    // Using imported gameState from globalExports
    ...
    if (['RB', 'LB'].includes(player.role)) {
        const wingerOnMySide = teammates.find(t => (player.role === 'RB' && t.role === 'RW') || (player.role === 'LB' && t.role === 'LW'));
