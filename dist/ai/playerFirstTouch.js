...
function calculateFirstTouchQuality(player, ballSpeed, nearbyOpponents) {
    ...
    const ballAngle = Math.atan2(gameState.ballPosition.y - player.y, gameState.ballPosition.x - player.x);
...
export function applyFirstTouch(player, trajectory, allPlayers) {
    const ballSpeed = trajectory ?
        Math.sqrt(Math.pow((trajectory.endX - trajectory.startX) / (trajectory.duration / 1000), 2) +
            Math.pow((trajectory.endY - trajectory.startY) / (trajectory.duration / 1000), 2)) : 200; // Default for loose balls
    const opponents = allPlayers.filter(p => p.isHome !== player.isHome);
    const nearbyOpponents = opponents.filter(opp => getDistance(player, opp) < FIRST_TOUCH_CONFIG.PRESSURE_DISTANCE * 1.5);
...
export function handleFailedFirstTouch(player, allPlayers) {
...
export function handlePoorFirstTouch(player, touchResult) {
...
export function handleSuccessfulFirstTouch(player, touchResult) {
...
export function canPlayerActOnBall(player) {
    if (!player.hasBallControl)
        return false;
...
export function drawFirstTouchIndicator(ctx, player) {
