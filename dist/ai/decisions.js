 ...
 */
export function calculateAvgDribbling(teamPlayers) {
 ...
 */
export function passBall(passingPlayer, fromX, fromY, toX, toY, passQuality = 0.7, speed = 400, isShot = false) {
...
export function calculateDribbleSuccess(player, opponents) {
    ...
    const composure = (player.composure || 60) / 100; // Default 60
...
export function calculatePassSuccess(passer, _receiver, distance, isUnderPressure) {
    ...
    const visionBonus = ((passer.vision || 60) / 100) * 0.15; // Default 60
    ...
    const composure = (passer.composure || 60) / 100; // Default 60
 ...
 */
export function checkForThroughBall(passer, teammates, opponents, opponentGoalX) {
 ...
 */
export function initiatePass(player, target) {
    if (!target) {
 ...
 */
export function initiateThroughBall(player, throughBall) {
    // Using imported gameState from globalExports
 ...
 */
export function initiateDribble(player, goalX) {
 ...
 */
export function handlePlayerWithBall_WithFirstTouch(player, opponents, teammates) {
    // Using imported gameState from globalExports
 ...
 */
export function handlePlayerWithBall_WithVision(player, opponents, teammates) {
    // Using imported gameState from globalExports
    ...
    if (!canPlayerActOnBall(player)) {
        ...
        player.speedBoost = 0.8; // Slow, controlled movement
    ...
    if (distToGoal < 280) {
        ...
        if (Math.abs(player.y - 300) > 150)
            shotValue *= 0.7; // Bad angle penalty
    }
