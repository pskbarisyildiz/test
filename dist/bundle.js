...
var FootballSim = (() => {
  ...
  function canPlayerSee(player, target, detailLevel = "full") {
    ...
    const nearbyOpponents = [...gameState.homePlayers, ...gameState.awayPlayers].filter((p) => p.isHome !== player.isHome && getDistance(player, p) < 50);
  ...
  function calculateDribbleSuccess(player, opponents) {
    ...
    const nearbyOpponents = opponents.filter((opp) => getDistance2(player, opp) < 40);
  ...
  function initiatePass(player, target) {
    ...
    const distance2 = getDistance2(player, target);
    const allPlayers = [...gameState.homePlayers, ...gameState.awayPlayers];
    const nearbyOpponents = allPlayers.filter((p) => p.isHome !== player.isHome && getDistance2(player, p) < 50);
  ...
  function initiateThroughBall(player, throughBall) {
    ...
    const quality = calculatePassSuccess(player, throughBall.target, getDistance2(player, throughBall.targetPos), false);
  ...
  function handlePlayerWithBall_WithFirstTouch(player, opponents, teammates) {
    ...
    const nearbyOpponents = opponents.filter((opp) => getDistance2(player, opp) < 50);
    ...
    const hasPathToGoal = !opponents.some((opp) => {
      ...
      return oppDistToGoal < distToGoal && Math.abs(opp.y - player.y) < 40 && getDistance2(player, opp) < 80;
    });
    const now = Date.now();
  ...
  function handlePlayerWithBall_WithVision(player, opponents, teammates) {
    if (!canPlayerActOnBall(player)) {
      ...
      for (let i = -1; i <= 1; i += 0.5) {
        ...
        const closestOpponentDist = opponents.length > 0 ? Math.min(...opponents.map((o) => getDistance2(o, checkPos))) : 1e3;
    ...
    let shotValue = 0;
    const distToGoal = getDistance2(player, { x: goalX, y: 300 });
    if (distToGoal < 280) {
      const xG = calculateXG(player, goalX, player.y, opponents);
      shotValue = xG * 100;
      if (opponents.some((o) => getDistance2(o, player) < 30))
        shotValue *= 0.6;
    ...
    let passValue = 0;
    if (passTarget) {
      const distToTarget = getDistance2(player, passTarget);
      const spaceAroundTarget = opponents.length > 0 ? Math.min(...opponents.map((o) => getDistance2(o, passTarget))) : 1e3;
    ...
    const spaceAhead = opponents.length > 0 ? Math.min(...opponents.map((o) => getDistance2(o, dribbleCheckPos))) : 1e3;
    if (spaceAhead > 40) {
  ...
  var DefensiveBehaviors = {
    ...
    fullBackCovering(fb, opponents) {
      ...
      const nearestThreat = wideThreats.sort(
        (a, b) => getDistance(a, fb) - getDistance(b, fb)
      )[0];
      if (!nearestThreat)
        return null;
    ...
    centerBackMarking(cb, opponents, ownGoalX) {
      ...
      const nearestStriker = strikers.sort(
        (a, b) => getDistance(a, cb) - getDistance(b, cb)
      )[0];
      if (!nearestStriker)
        return null;
  ...
  var ForwardBehaviors = {
    ...
    invertedWingerCutInside(winger, opponentGoalX) {
    ...
    strikerPressingTrigger(striker, opponents, ball, ownGoalX) {
      ...
      const nearestCB = oppCBs.sort(
        (a, b) => getDistance(a, ball) - getDistance(b, ball)
      )[0];
      if (!nearestCB)
        return null;
  ...
  var TransitionBehaviors = {
    counterAttackRun(player, opponentGoalX, justWonPossession) {
  ...
  var TacticalSystemModifiers = {
    possessionSystem(player, holder, teammates) {
      ...
      const nearbyTeammates = teammates.filter(
        (t) => getDistance(t, holder) < 150 && t.id !== player.id && t.id !== holder.id
      );
  ...
  var MovementPatterns = {
    ...
    thirdManRun(player, holder, teammates) {
      ...
      const passerAndReceiver = teammates.filter(
        (t) => getDistance(t, holder) < 80 && t.id !== holder.id
      );
  ...
  function calculateXG2(shooter, goalX, goalY, opponents) {
    ...
    const normalizedDistance = Math.min(distToGoal / 400, 1);
    const distanceQuality = Math.pow(1 - normalizedDistance, 1.5);
    const nearbyDefenders = opponents.filter((opp) => getDistance3(shooter, opp) < 30);
    const pressureMultiplier = Math.max(0.4, 1 - nearbyDefenders.length * 0.18);
    const shooterAbility = 0.2 + shooter.shooting / 100 * 0.8;
    const speed = Math.sqrt(shooter.vx * shooter.vx + shooter.vy * shooter.vy);
    const movementPenalty = speed > 100 ? 0.9 : 1;
  ...
  function updateMatchStats() {
    const now = Date.now();
    const elapsed = (now - gameState.stats.lastPossessionUpdate) / 1e3;
  ...
  function assessGoalkeeperThreats(goalkeeper, _ball, opponents) {
    ...
    const threats = opponents.filter((opp) => opp.role !== "GK").map((opp) => {
      ...
      const distToGK = getDistance4(goalkeeper, opp);
  ...
  function shouldGoalkeeperSweep(goalkeeper, _ball, opponents) {
    ...
    const keeperDistToBall = getDistance4(goalkeeper, { x: traj.endX, y: traj.endY });
    const attackerDistToBall = Math.min(...attackersChasingBall.map(
      (att) => getDistance4(att, { x: traj.endX, y: traj.endY })
    ));
    const realStats = goalkeeper.realStats || {};
  ...
  function handleCrossSituation(goalkeeper, _ball, _opponents) {
    ...
    const distToBall = getDistance4(goalkeeper, { x: traj.endX, y: traj.endY });
    const realStats = goalkeeper.realStats || {};
  ...
  function updateGoalkeeperAI_Advanced(goalkeeper, ball, opponents) {
    ...
    if (crossAction) {
      ...
      setTimeout(() => {
        ...
        if (goalkeeper.isClaimingCross && goalkeeper.crossClaimStartTime === claimStartTime && getDistance4(goalkeeper, gameState.ballPosition) < 40) {
  ...
  function assessDefensiveThreats(defendingPlayer, opponents, ownGoalX) {
    ...
    return opponents.map((opponent) => {
      ...
      const distToDefender = getDistance5(defendingPlayer, opponent);
      ...
      const nearbyDefenders = teammates.filter((d) => getDistance5(d, opponent) < 60);
  ...
  function applyMarkingAndPressing(player, _ball, opponents, activePosition, ownGoalX, tactic, teamState) {
    ...
    const distToBallCarrier = getDistance5(player, ballCarrier);
    ...
    const isClosestDefender = !allDefenders.some((p) => getDistance5(p, ballCarrier) < distToBallCarrier);
  ...
  function applyDefensivePositioning(player, ball, _tactic, activePosition, ownGoalX, teamState) {
    ...
    if (["RB", "LB"].includes(playerRole)) {
      ...
      const myNearestCB = centralDefenders.sort((a, b) => getDistance5(a, player) - getDistance5(b, player))[0];
      if (myNearestCB) {
    ...
    if (["RB", "LB"].includes(playerRole) && isBallOnFarSide) {
      ...
      if (centerBacks.length > 0) {
        const nearestCB = centerBacks.sort((a, b) => getDistance5(a, player) - getDistance5(b, player))[0];
        if (nearestCB) {
  ...
  function applyAttackingMovement(player, holder, teammates, activePosition, opponentGoalX, _teamState) {
    ...
    if (["LW", "RW"].includes(player.role) && ballOnOtherSide && getDistance5(holder, player) > 150) {
      return {
    ...
    const runOptions = [];
    const holderUnderPressure = opponents.some((o) => getDistance5(o, holder) < 70);
    if (holderUnderPressure) {
      const supportTarget = {
        x: holder.x - direction * 60,
        y: holder.y + (player.y < 300 ? -70 : 70)
      };
      const supportSpace = Math.min(...opponents.map((o) => getDistance5(o, supportTarget)));
    ...
    if (["ST", "RW", "LW", "CAM"].includes(player.role)) {
      ...
      if (lastDefender) {
        ...
        const spaceRating2 = Math.min(...opponents.map((o) => getDistance5(o, runTarget)));
    ...
    const spaceRating = Math.min(...opponents.map((o) => getDistance5(o, spaceRunTarget)));
    ...
    if (player.role === "CM" || player.role === "CAM") {
      ...
      if (striker) {
        const distToStriker = getDistance5({ x: finalMove.x, y: finalMove.y }, striker);
        if (distToStriker < 80) {
  ...
  function shouldLockSetPiecePosition(player, gameState2) {
    ...
    if (lockKeywords.some((keyword) => movementLower.includes(keyword))) {
      return getDistance(player, gameState2.setPiece.position) < 15;
    }
  ...
  function canControlBall(player, ball) {
    ...
    if (isSetPiece && gameState.setPiece && !gameState.setPiece.executed) {
      ...
      const isGoalkeeper2 = player.role === "GK" || player.role === "goalkeeper";
  ...
  function calculateFirstTouchQuality(player, ballSpeed, nearbyOpponents) {
    ...
    const speedDifficulty = Math.min(
      (ballSpeed - FIRST_TOUCH_CONFIG.SLOW_PASS_SPEED) / (FIRST_TOUCH_CONFIG.FAST_PASS_SPEED - FIRST_TOUCH_CONFIG.SLOW_PASS_SPEED),
      1
    );
    const speedPenalty = speedDifficulty * 0.25;
    const opponentsInRange = nearbyOpponents.filter(
      (opp) => getDistance(player, opp) < FIRST_TOUCH_CONFIG.PRESSURE_DISTANCE
    );
  ...
  function applyFirstTouch(player, trajectory, allPlayers) {
    ...
    const opponents = allPlayers.filter((p) => p.isHome !== player.isHome);
    const nearbyOpponents = opponents.filter(
      (opp) => getDistance(player, opp) < FIRST_TOUCH_CONFIG.PRESSURE_DISTANCE * 1.5
    );
