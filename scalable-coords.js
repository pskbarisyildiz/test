...
const FIELD_CONFIG = {
    ...
    distanceInMeters(dist) {
        // Convert pixel distance to approximate real meters
        return (dist / this.WIDTH) * 105;
    },
    
    speedToPixelsPerSecond(metersPerSecond) {
        // Convert m/s to pixels/second
...
// Stores positions as percentages, converts to pixels when needed
// ============================================================================

class RelativePosition {
    constructor(x, y) {
        // Store as 0-1 coordinates
        this.rx = x; // relative x
        this.ry = y; // relative y
    }
    
    // Get pixel coordinates
    get x() { return this.rx * FIELD_CONFIG.WIDTH; }
    get y() { return this.ry * FIELD_CONFIG.HEIGHT; }
...
// ============================================================================
// PITCH GEOMETRY - All key positions in relative coordinates
// ============================================================================
...
const PITCH_GEOMETRY = {
    ...
    // Penalty box boundaries (left side)
    leftPenaltyBox: {
    ...
    // Penalty box boundaries (right side)
    rightPenaltyBox: {
...
// ============================================================================
// PLAYER POSITION MANAGER - Handles relative positioning for players
// ============================================================================
...
class PlayerPositionManager {
    constructor(player) {
        ...
        // Velocity in relative coordinates per second
        this.velocity = { rx: 0, ry: 0 };
    }
    
    // Update from legacy pixel coordinates (for backwards compatibility)
    updateFromPixels(x, y) {
    ...
    // Move toward target with realistic acceleration
    moveToward(targetRelPos, deltaTime, speedMultiplier = 1.0) {
        const maxSpeed = FIELD_CONFIG.speedToPixelsPerSecond(
            this.player.pace / 100 * 10 // 10 m/s max for fast player
        ) * speedMultiplier;
        
        // Convert to relative speed
        ...
        if (distance < 0.001) return; // Already there
        
        // Acceleration
        const acceleration = maxRelSpeed * 5; // Reach max speed in ~0.2s
    ...
    // Set home position (formation position)
    setHomePosition(relX, relY) {
        this.homePosition = new RelativePosition(relX, relY);
    }
}

// ============================================================================
// FORMATION POSITIONS - All formations in relative coordinates
// ============================================================================

const SCALABLE_FORMATIONS = {
    '4-3-3': [
...
// Example 1: Initialize player at formation position
function initializePlayerPosition(player, formationPos) {
    player.positionManager = new PlayerPositionManager(player);
    player.positionManager.setHomePosition(formationPos.rx, formationPos.ry);
    player.positionManager.position = player.positionManager.homePosition.clone();
}

// Example 2: Move player to target
function updatePlayerMovement(player, deltaTime) {
    if (!player.positionManager) return;
    
    // Target is stored in relative coordinates
    const target = player.positionManager.targetPosition;
    player.positionManager.moveToward(target, deltaTime, player.speedBoost || 1.0);
    
    // Update legacy pixel coordinates for rendering
    const pixelPos = player.positionManager.getPixelPosition();
    player.x = pixelPos.x;
    player.y = pixelPos.y;
}

// Example 3: Check if player is in attacking third
function isInAttackingThird(player, isHome) {
    const pos = player.positionManager.position;
...
// Example 4: Scale the entire field
function setFieldScale(newScale) {
    FIELD_CONFIG.SCALE_FACTOR = newScale;
    
    // All canvases auto-resize
    document.getElementById('gameCanvas').width = FIELD_CONFIG.WIDTH;
    document.getElementById('gameCanvas').height = FIELD_CONFIG.HEIGHT;
    
    // All positions automatically scale - no recalculation needed!
}
...
console.log('✅ V2 Scalable Coordinate System Loaded');
console.log(`📐 Field: ${FIELD_CONFIG.WIDTH}x${FIELD_CONFIG.HEIGHT}px`);
console.log(`🎯 Change SCALE_FACTOR to resize everything instantly!`);