function updateBallTrajectory(dt) {
...
function validateBallState() {
    ...
    if (window.DEBUG_PHYSICS && (oldX !== gameState.ballPosition.x || oldY !== gameState.ballPosition.y)) {
...
function updateBallWithHolder(allPlayers, dt) {
    ...
    if (holder && allPlayers.includes(holder)) {
        const facingAngle = getPlayerFacingDirection(holder);
        ...
        // DÜZELTME: BALL_PHYSICS global olmalı (örn: GAME_CONFIG.BALL_PHYSICS)
        // Varsayılan değerler kullanılıyor:
        const FRICTION = (typeof BALL_PHYSICS !== 'undefined') ? BALL_PHYSICS.FRICTION : 0.985;
        ...
        // DÜZELTME: Hata 3 (Döngüsel Bağımlılık) çözümü için bu fonksiyonlar main.js'e taşındı.
        if (gameState.ballPosition.x < 5 || gameState.ballPosition.x > 795) {
            if (typeof handleBallOutOfBounds === 'function') handleBallOutOfBounds();
            return;
        }

        if (gameState.ballPosition.y < 5 || gameState.ballPosition.y > 595) {
            if (typeof handleThrowIn === 'function') handleThrowIn();
            return;
        }
...
function updatePlayerPhysics(allPlayers, dt) {
    // DÜZELTME: PHYSICS global olmalı (örn: GAME_CONFIG.PHYSICS)
    // Varsayılan değerler kullanılıyor:
   ...
   const PHYSICS = (typeof window.PHYSICS !== 'undefined') ? window.PHYSICS : PHYSICS_DEFAULTS;;
    ...
    allPlayers.forEach(player => {
        ...
        if (dist > PHYSICS.MOVEMENT_THRESHOLD) {
            ...
            player.distanceCovered += speed * dt;
            ...
            const speed = Math.sqrt(player.vx * player.vx + player.vy * player.vy);
            let recoveryRate = speed < 10 ? 1.2 : 0.6; 
...
function assignBallChasers(allPlayers, priorityChaser = null) {
    ...
    const chaseEvaluations = allPlayers
        .filter(p => p.role !== 'GK') 
        .map(player => {
            ...
            if (player.stamina < 40) { score *= 0.7; }
...
function handleGoalkeeperBallHold(holder, allPlayers) {
    ...
    const GK_HOLD_TIME = (typeof GAME_CONFIG !== 'undefined') ? GAME_CONFIG.GK_HOLD_TIME : 5000;
    ...
    if (holder.ballReceivedTime && holder === gameState.ballHolder) {
        ...
        if (holdTime > GK_HOLD_TIME) {
            ...
            if (target) {
                passBall(holder, holder.x, holder.y, target.x, target.y, 0.9, 450, false);
...
function updatePlayerVelocity(player, dx, dy, dist, dt) {
    // ✅ FIX #2: SAFE MATH GUARDS - Prevent NaN propagation
    if (!isFinite(dx) || !isFinite(dy) || !isFinite(dist) || dist === 0) {
        return;
    }

    // Use centralized config access
    const PHYSICS = (typeof window.PHYSICS !== 'undefined') ? window.PHYSICS : {
        ACCELERATION: 1200,
        MAX_SPEED: 220,
        DRIBBLE_SPEED_PENALTY: 0.8
    };
    ...
    let accel = PHYSICS.ACCELERATION * (paceFactor + physicalityBonus) * speedMultiplier;
    ...
    // ✅ FIX #2: Use safe division with MathUtils
    const dirX = (typeof MathUtils !== 'undefined') ? MathUtils.safeDiv(dx, dist, 0) : dx / dist;
    const dirY = (typeof MathUtils !== 'undefined') ? MathUtils.safeDiv(dy, dist, 0) : dy / dist;
    ...
    // ✅ FIX #2: Validate velocity before calculating speed
    const currentSpeed = (typeof MathUtils !== 'undefined')
        ? MathUtils.safeSqrt(player.vx * player.vx + player.vy * player.vy, 0)
        : Math.sqrt(player.vx * player.vx + player.vy * player.vy);

    player.currentSpeed = currentSpeed;

    // ✅ FIX #2: Safe velocity clamping
    if (currentSpeed > effectiveMaxSpeed && currentSpeed > 0) {
        const ratio = (typeof MathUtils !== 'undefined')
            ? MathUtils.safeDiv(effectiveMaxSpeed, currentSpeed, 1)
            : effectiveMaxSpeed / currentSpeed;
        player.vx *= ratio;
        player.vy *= ratio;
    }
}
function applyPlayerFriction(player, dt) {
    ...
    const PHYSICS_FRICTION = (typeof PHYSICS !== 'undefined') ? PHYSICS.FRICTION : 0.88;
...
function updateCameraShake(dt) {
    if (gameState.cameraShake > 0) {
        gameState.cameraShake -= gameState.cameraShake * 18 * dt;
        if (gameState.cameraShake < 0.1) gameState.cameraShake = 0;
    }
}


// Export critical functions to global scope for game loop access
