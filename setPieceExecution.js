...
function executeCornerKick_Enhanced(gameState) {
...
function executeFreeKick_Enhanced(gameState) {
     if (!gameState || !gameState.setPiece || !gameState.setPiece.position) return false;
     const setPiece = gameState.setPiece;
     const takingTeam = setPiece.team;
     const allPlayers = [...(gameState.homePlayers || []), ...(gameState.awayPlayers || [])];
...
function executeThrowIn_Enhanced(gameState) {
     if (!gameState || !gameState.setPiece || !gameState.setPiece.position) return false;
     const setPiece = gameState.setPiece;
     const takingTeam = setPiece.team;
     const allPlayers = [...(gameState.homePlayers || []), ...(gameState.awayPlayers || [])];
     ...
     let thrower = takingTeamPlayers.find(p =>
         gameState._throwInAssignments?.get(String(p.id))?.role === 'THROWER'
     );
...
function executeGoalKick_Enhanced(gameState) {
    // --- 1. SETUP (Gerekli) ---
    ...
    // Taktik bilgisini al
    const tacticKey = takingTeamIsHome ? (gameState.homeTactic || 'balanced') : (gameState.awayTactic || 'balanced');
    ...
    console.log(`⚽ Goal Kick: ${playShort ? 'Kısa' : 'Uzun'} oyun planı (Taktik: ${tacticKey})`);
    ...
    if (playShort && Math.random() < 0.75) {
        ...
        if (shortTargets.length > 0 && shortTargets[0].space > 30) {
    ...
    if (longTargets.length > 0 && Math.random() < 0.7) {
...
function executeKickOff_Enhanced(gameState) {
...
function executeSetPiece_Router(gameState) {
    if (!gameState || !gameState.setPiece) {
